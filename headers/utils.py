...
def cross_validation(model, X, y,i):

    print("Model : ",str(model.__class__.__name__)+str(i))
    start = time()
    ...
    print(
        f"Mean accuracy: {accuracy[str(model.__class__.__name__)]}\nStd: {np.std(score):.3f}\nRun time: {speed[str(model.__class__.__name__)]}s"
    )
    print("\n\n\n\t")
    try:
        print("Predict proba", str(model.predict_proba(X_test))+str(i))
    except:
        print(f"Can't predict likelihood for {model.__class__.__name__}")
